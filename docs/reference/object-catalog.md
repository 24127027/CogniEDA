...
| --- | --- | ---: | --- | --- | --- |
| planning and plan versioning | `PlanRevision`, `PlanTaskBinding`, `PlanTaskDependency`, `TaskLifecycleRecord`, `TaskPresentationMetadata`, `PlannerConsultationRun` | no | Planner proposes immutable plan content and coordination; application authority admits it, while separate workflow lifecycle owns approval, activation, completion, interruption, and replanning facts | versioned content plus separate lifecycle-governed state | [Task and PlanRevision](../concepts/research-state/planning-and-scientific-state.md#task-is-work-planrevision-is-a-plan) |
