...
- `SessionContextBuilder` projects a frame into typed planning and conclusion bundles.
- Planning context may include active assumptions.
- Conclusion context excludes assumptions, tasks, user decisions, pending questions, stale context, dead ends, and cached tool-result summaries.
...
| Context Type Safety | Partially implemented | Basic frame projection enforces planning-vs-conclusion exclusions, but no typed graph retrieval policy exists. |

## Known Deviation

Direct prompt construction from raw `SessionFrame` snapshots would bypass the conclusion-context guard. Conclusion generation should use `SessionContextBuilder` or an equivalent graph retrieval policy.
