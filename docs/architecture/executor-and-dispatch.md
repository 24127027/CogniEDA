...
provider factory, and dispatcher. Planner receives the dispatcher only as a
model-hidden `PlannerDeps` service and never selects Capability. The active
`run_data_work` semantic tool validates exact approved Plan membership and DAG
eligibility before internally translating DATA work to current execution
plumbing. Scientific and graph semantic tools remain **Deferred**.
