...
| `Assumption` | Partially implemented | Schema, table, and repository exist. Current statuses are `active`, `validated`, `rejected`, `archived`, which differ from the target lifecycle. Testability admission and conclusion-context exclusion are not enforced. |
