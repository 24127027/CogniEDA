...
| ExecutionRun | Valid provenance | Durable non-FCO attempt record exists with identity, status, lease/fencing, retry and recovery fields. It remains narrower than full reproducibility provenance. |
| PlannerOperation | Valid workflow/provenance | Durable non-FCO envelope/table/repository and local atomic commit/rollback boundary exist. Handler/reachability gaps remain. |
