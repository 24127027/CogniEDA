...
| Execution/Evidence/Evaluation/Governance bounded contexts | Execution coordination, Evidence admission, protected evaluation, governance decision authority, and canonical schemas have separate owners | `application.execution`, `application.evidence`, `application.evaluation`, `application.governance`, `schemas.evaluation`, `schemas.governance`, `repositories.evaluation`, and `repositories.governance` are active; old orchestrator/schema/repository paths are removed | Implemented by S1-B & S2-A; remaining Discovery admission and validity decomposition is deferred to S2-B/S3 |
