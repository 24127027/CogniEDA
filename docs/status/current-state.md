...
| Conversational Plan authorization and candidate lifecycle | **Implemented** | In-process `PlannerRuntime` graph state retains an exact Plan/Task candidate and native model history outside `PlannerContext`. Natural-language Human turns resume through LangGraph interrupt state; typed Planner results retain, replace, explicitly discard, or authorize the exact candidate without keyword or regex parsing. Authorization calls `PlanAdmissionService`; success clears the candidate, while failure retains it with a controlled error. Exact admission and objective-scoped active Plan selection are **Verified on SQLite**. Checkpoints are process-local and durable restart recovery is **Deferred**. |
...
| Retained runtime composition | **Partially implemented** | Application composes one process-local Planner lifecycle with fresh context, native history, candidate review, interrupt/resume, exact admission, and presentation events. No supported Planner-to-real-Data-Explorer-to-Evidence-to-SessionFrame workflow is composed, and frame/graph state is not restored after restart. |
...
| Runtime entry boundary | **Partially implemented** | Runtime bootstrap composes fresh-SQLite metadata and session access, delegation infrastructure, Planner, `PlannerRuntime`, model factory, EventBus, and workspace-local agent tooling. One in-process Application owns the current SessionFrame and runtime UUID; graph state owns native history and candidate lifecycle, while a provider resolves the exact active Plan into fresh `PlannerContext`. `Application.submit_message()` delegates normal text to the graph and publishes typed presentation events from its turn outcome. Commands use the same EventBus boundary. EventBus stores no research state or candidate authority. Typed authorization invokes application-owned admission; `continue_execution` for an already-active Plan returns a visible no-execution result and never dispatches. Persisted graph state is not recovered and no Plan execution loop exists, so this is not a supported product CLI. |
...
model-visible Capability. Runtime tests prove exact graph-state and runtime-
context shapes, candidate retain/replace/discard, multi-turn natural-language
authorization, real SQLite admission exactly once, controlled admission failure
retention, interrupt/re-interrupt behavior, thread isolation, fresh
`PlannerContext`, and visible active-Plan deferral without dispatch. Event tests
...
Result: **72 passed**. Changed Python files also pass Ruff, strict mypy, and
`compileall`. This is changed-boundary evidence, not a claim that deferred Plan
execution, durable lifecycle recovery, or scientific workflows exist.
