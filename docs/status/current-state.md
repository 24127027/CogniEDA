...
| MVP SessionFrame | **Implemented** | The frozen schema is a typed reference manifest containing `objective_id`, ordered `assumption_ids`, `task_ids`, and `evidence_ids`, and optional `data_profile_id`; it stores no materialized research objects or conversation. Duplicate reference IDs fail validation. `BuildPlanningContext` resolves current objects and fails closed on missing references, orphan Evidence, non-`COMPLETED` authoritative Tasks, missing DataProfile, and profile mismatch. Frame membership itself grants neither validity nor authority. |
| Runtime Session and conversation continuity | **Implemented** | At the in-process `Application` boundary, one immutable successor `Session` owns `SessionFrame` and append-only `ConversationHistory` separately. Conversation turns preserve native PydanticAI `ModelMessage` sequences, including coherent tool call/return structure, and round-trip through Pydantic serialization. Request understanding receives selected history through PydanticAI `message_history`; empirical answer input excludes conversation and Assumptions. |
...
| M5-A runtime composition | **Partially implemented** | In-process SessionFrame and Human-to-Planner conversation continuity are retained across `submit_message()` calls. The runtime still does not compose authoritative dataset execution context, M3-A Evidence admission, active-profile selection, or restart-safe recovery into the user workflow. |
...
SessionFrame(objective_id, assumption_ids, task_ids, evidence_ids, data_profile_id)
```
...
removed; the current `BuildPlanningContext` instead resolves the active ID-only
SessionFrame through authoritative object repositories for the four-node graph.
