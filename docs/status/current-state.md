...
| Objective and Assumption | **Implemented** | Immutable `Objective` and planning-only `Assumption` each have stable UUID identity and non-empty text. Planner may return a transient candidate Plan containing the current or a newly proposed Objective, but it cannot create an Assumption. Planner-interpreted Human authorization of the exact pending Plan may atomically admit its new Objective; every Plan Assumption must already exist with exact identity and content and is never auto-created by Plan admission. |
...
| Conversational Plan authorization and admission | **Verified on SQLite** | Application retains one exact transient candidate bundle and exposes it to the next Planner call separately from authoritative Tasks and the active Plan. Planner interprets a later Human prompt. Clear authorization returns `continue_execution`; Application then admits the exact bundle that was pending before that invocation, requiring unchanged admitted Assumptions, exact Objective and Task identities, no Plan collision, and one atomic persistence-and-activation transaction. The same returned candidate is retained, a new candidate replaces it, and a response without a candidate clears it. A candidate cannot be created and admitted in one invocation. Conversation text is not direct Application authority, and pending state is not durable. |
...
| Phase 2 Planner cognitive core and native conversation history | **Implemented** | Planner directly owns one typed PydanticAI Agent and invokes `plan_or_answer` exactly once per non-empty request with exact typed `PlannerDeps`. `PlannerResult` can answer, request Human clarification, propose a transient Plan plus exact Task bundle, or signal continuation of a supplied pending or active Plan. `PlannerOutput` contains that result, only native messages generated by the current invocation, and an optional controlled error. Prior `ConversationHistory` is supplied as native message history and remains non-authoritative. Candidate validation fails closed on Task-bundle mismatch, unknown or changed Assumptions, and continuation without a pending or active Plan. The model-visible contracts contain no Capability or execution routing. Planner performs no persistence, admission, activation, dispatch, or Evidence admission. |
...
| Canonical planning and scientific runtime | **Deferred** | Plan domain, pure validation, transient candidate authoring, Planner-interpreted Human authorization, atomic exact-bundle admission, and objective-scoped active selection exist. No active Plan executes: Task DAG selection/execution, successor/replanning orchestration, and final LangGraph composition remain absent, and Plan binds no exact DataProfile. The scientific runtime, canonical Evidence admission, protected evaluation, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
| Plan lifecycle and replanning | **Partially implemented** | Conversational authorization of an exact pre-existing pending candidate and objective-scoped active selection are implemented, and successor activation does not mutate the old Plan. Plan completion, interruption, successor-authoring policy, pending-candidate recovery, and actual-cause taxonomy remain **Deferred**. Scientific stopping remains InvestigationProtocol-owned, and bounded execution stopping remains work-order-owned. |
...
| Runtime entry boundary | **Partially implemented** | Runtime bootstrap composes fresh-SQLite metadata, the registry, Data Explorer provider, dispatcher, Phase 2 Planner, model factory, and workspace-local agent tooling. One in-process `Application` owns the current SessionFrame, exact transient candidate, and native conversation history. It materializes pending Plan/Tasks separately from authoritative state and resolves the active Plan for the frame's exact current Objective into `PlannerContext`. `continue_execution` admits a pre-existing pending candidate or remains a no-execution semantic result for an already-active Plan. Persisted state is not recovered and no Plan execution loop exists, so this is not a supported product CLI. |
...
candidate authoring, conversational authorization, commit-boundary validation
and atomic persistence, and objective-scoped activation are **Implemented** and
**Verified on SQLite** at their bounded surfaces. Lifecycle progression,
durable pending-candidate recovery, and runtime execution remain **Deferred**.
...
Runtime tests prove that candidate state is not admitted in its authoring call,
pending Tasks remain separate from authoritative Tasks, the same or replacement
candidate is retained exactly, abandonment clears it, and continuation admits
only the candidate that preceded the accepting Human prompt. They also prove
atomic admission and that the exact objective-scoped active Plan reaches
`PlannerContext` alongside all six materialized SessionFrame member categories.
Layer tests
...
Result: **118 passed**. This is changed-boundary evidence, not a claim that the
