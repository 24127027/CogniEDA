...
| Runtime Session and conversation continuity | **Implemented** | At the in-process `Application` boundary, one immutable successor `Session` owns `SessionFrame` and complete `ConversationHistory` separately. Each `ConversationTurn` retains Human/Planner surface text plus zero or more native `ConversationSegment` retention units. CogniEDA validates segment identity, non-emptiness, and aggregate ID uniqueness but does not reimplement PydanticAI's tool/retry protocol. Deterministic Unicode-aware selection returns the four most recent turns plus at most four most recent older lexical matches, chronologically; native messages are derived from those turns without a duplicate selected-message field. Omission does not delete history, and deterministic turns fabricate no model messages. Request understanding receives surface text through an explicitly non-authoritative typed discourse field and native messages through PydanticAI `message_history`; empirical answer input excludes conversation and Assumptions. |
