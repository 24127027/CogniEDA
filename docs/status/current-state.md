...
| Objective and Assumption | **Implemented** | `Objective` and planning-only `Assumption` each have stable UUID identity and non-empty text. A candidate `Plan` owns its exact Objective value and a selected basis of exact already-admitted Human Assumptions. Human Plan approval also approves a new Objective in that Plan, but never admits an Assumption. Missing Assumption identity or same identity with different content fails closed. Planner-side Assumption assessment/admission is **Deferred**; model-generated possibilities remain reasoning or may motivate Tasks or Human clarification. Assumptions guide planning only and never become empirical premises. |
...
| Data Explorer | **Implemented** | At the bounded M3-A library surface, execution-internal capability plumbing invokes Data Explorer behind an approved semantic Planner tool. Data Explorer rejects direct `SCIENTIFIC` and `GRAPH` Task dispatch. It owns typed operationalization of model-supplied semantic `requested_work` plus the authoritative DataProfile projection into one finite `DataAnalysisPlan`; deterministic validation and tools alone compute values. Supported operations are row count, column summary, missingness, bounded value counts, descriptive statistics, bounded group summary, and bounded Pearson or Spearman correlation. Exact column names are required. Analysis contracts live under `agents.data_explorer`, not role-neutral `execution`. Data Explorer remains persistence-free and never itself creates Evidence. General-purpose Python execution is **Unsupported**. Transformation remains a typed blocker. |
| Planner behavior and native conversation history | **Implemented** | Planner owns one PydanticAI `Agent` and exactly two LangGraph cognitive nodes, `plan_or_answer` and `execute`. `plan_or_answer` may answer, propose a new Plan, request Human clarification, or set `continue_execution`; executor tools are structurally hidden there. A candidate Plan pauses at a LangGraph interrupt; a newly approved Plan resumes with the exact authoritative active Plan, while continuing an unchanged active Plan requires no new approval. `execute` uses the same Agent and semantic tools, produces no `PlannerResult`, and always returns to `plan_or_answer`. One optional-field `PlannerResult` and one `PlannerOutput(result, messages, error)` envelope carry the Human-boundary snapshot. `messages` retains ordered model, tool-call, tool-return, and explicit Human review messages across the complete lifecycle, which Application appends once as one `ConversationTurn`. `PlannerContext` includes the active Plan plus complete readable research state and non-authoritative `ConversationHistory`. |
| M3-A Data Explorer execution and Evidence integration | **Implemented** | The bounded library/data-authority surface proves immutable path-plus-content DataProfile binding, Data Explorer-owned bounded planning, deterministic real tool execution, typed provenance with the actual execution digest, application-owned atomic profile/binding admission, and application-owned immutable Evidence admission. `PlannerExecutionSession` re-resolves the persisted active Plan and complete current Task bundle at each tool call, recomputes dependency eligibility, dispatches Data Explorer, and atomically commits Task completion with Evidence before returning that Evidence to Planner. The typed successor `PlannerContext` exposes the new Task status and Evidence without a mandatory full context reload. Exact replay is idempotent; wrong-path, wrong-profile, same-path mutation, planning failure, blocker, lineage mismatch, and invalid payload paths create zero Evidence. |
...
| Retained runtime composition | **Partially implemented** | The in-process Planner execution session composes approved DATA tool work through Data Explorer, application Evidence admission, successor PlannerContext, and Application adoption into the materialized SessionFrame. The development entrypoint still has no supported Human dataset-adoption flow, and the current frame, active Plan, pending lifecycle, and conversation are not restored after restart; therefore this is not a supported complete user workflow or MVP-v2. |
...
| Plan lifecycle and replanning | **Partially implemented** | Candidate presentation, exact approve/reject/revise identity, atomic activation, `continue_execution`, and the fixed `execute -> plan_or_answer` return are implemented in process. Every replacement Plan requires another Human interrupt; continuing the unchanged active Plan does not. Plan content contains no configurable stopping-condition or replan-trigger policy, and execute emits no Planner decision field. Durable recovery, typed completion/interruption records, successor lineage, and the finite taxonomy of actual reconsideration causes remain **Deferred**. |
...
Data Explorer passes the semantic requested work, DataProfile/schema, and finite
...
The standalone admission service does not mutate `SessionFrame` directly. In
the composed Planner execution path, its admitted Evidence and completed Task
are applied to a typed successor `PlannerContext`, which Application adopts into
the current materialized frame. Canonical typed-reference membership,
active-profile selection, and restart reconstruction remain **Deferred**.
...
Plan review, exact `PlannerDeps` phase state, dynamic tool omission during
`plan_or_answer`, semantic tool visibility during `execute`, approved membership
and dynamically recomputed DAG eligibility, controlled dispatch failure,
multiple tool calls in one Agent run, admitted Evidence return, successor
context propagation, and retention of native tool-call, tool-return, and Human
review messages. Layer tests verify that
...
uv run pytest tests/agents/planner tests/agents/test_instruction.py \
  ...
  tests/application/services/test_planner_execution.py \
