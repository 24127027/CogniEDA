...
`ff5ad29234cf3d795cd06437b0ef9e88a9f032fb`. A
...
| Canonical planning and scientific runtime | **Deferred** | PlanRevision domain records, one-binding-per-member Task membership, binding-owned capability/role/rank/priority, DAG validation, and structural fingerprinting are not implemented. PlanRevision binds no exact DataProfile; future specialists select from complete authoritative context and future downstream provenance exact-binds what was used. Task DAG approval/activation, executable `SCIENTIFIC`, `GRAPH`, and `SYNTHESIS` workflows, specialist DataProfile-selection plumbing, ScientificInvestigationRun, Hypothesis Analyst feasibility, Hypothesis admission, InvestigationPlan/Protocol, EvidenceRequest, ExecutionRun, AnalysisFrame, canonical Evidence admission, protected evaluation, typed outcomes, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
