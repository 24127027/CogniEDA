...
| Objective and Assumption | **Implemented** | Immutable `Objective` and planning-only `Assumption` each have stable UUID identity and non-empty text, which prevents nested mutation from changing an existing Plan fingerprint. Bounded Planner returns explicit `created_objective` and `created_assumption` results: semantic Objective refinement allocates a new identity, and Application applies either result through the immutable SessionFrame successor seams. Durable behavioral persistence remains outside this boundary. |
...
| Plan V1 domain contract and candidate validation | **Implemented** | Immutable non-FCO `Plan`, `PlanTaskBinding`, and `PlanDependency` values contain the exact Objective, exact admitted Human Assumption planning basis, one binding per independent Task FCO, binding-owned non-negative tie-permitting order rank/LOW-NORMAL-HIGH priority, and explicit acyclic dependencies. Deterministic `sha256:` fingerprint content is exactly contract version, Objective and Assumption representations, Task membership/rank/priority, and dependency edges; Task runtime status and execution routing are excluded. The side-effect-free `PlanValidator` requires exact persisted Objective and Assumption content, resolves persisted Tasks, enforces exact membership and Objective scope, verifies canonical representation and fingerprint, and performs no persistence, Human interaction, capability/provider lookup, or execution selection. |
...
| Canonical planning and scientific runtime | **Deferred** | The Plan V1 domain contract, side-effect-free persisted-reference validation, and append-only SQLite repository foundation exist, but Planner authoring, Human approval, commit-boundary persistence, active Plan selection, activation, Task DAG execution, and successor/replanning runtime do not. Validation performs no authoritative persistence side effect, and no Plan candidate is persisted through an application path. Plan binds no exact DataProfile. Executable `SCIENTIFIC` and `GRAPH` workflows, specialist DataProfile-selection plumbing, ScientificInvestigationRun, Hypothesis Analyst feasibility, Hypothesis admission, InvestigationPlan/Protocol, EvidenceRequest, canonical ExecutionRun/AnalysisFrame use, canonical Evidence admission, protected evaluation, typed outcomes, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
...
snapshots while referenced FCO existence still fails closed. Planner authoring, human
approval, commit-boundary validation and persistence, activation, active
selection, lifecycle progression, and runtime use remain **Deferred**.
...
loaders, Data Explorer implementations, or deferred scientific and Plan
contracts or SessionFrame directly. Architecture tests also prove
