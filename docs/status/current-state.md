...
| PlanRevision V1 domain contract | **Implemented** | Immutable non-FCO `PlanRevision`, `PlanTaskBinding`, and `PlanDependency` values provide one binding per member Task, binding-owned required capability/non-negative tie-permitting order rank/LOW-NORMAL-HIGH priority, explicit Objective-scoped acyclic dependencies, deterministic canonical ordering, and a `sha256:` fingerprint bound to contract version and plan content. Construction requires authoritative Task objects and rejects duplicates, missing or cross-Objective Tasks, invalid kind/capability combinations, non-member or cyclic edges, and extra fields. Execution-owned `Capability` is the only routing requirement; `ExecutorRegistry` owns provider resolution. PlanRevision and its bindings contain no role or provider identity, DataProfile identity, stopping/replan policy, approval, or activation state. `SYNTHESIS` requires no capability and has no executable provider path. |
...
| Canonical planning and scientific runtime | **Deferred** | The PlanRevision V1 domain contract, exact SQLite persistence, and authoritative proposal admission are implemented, but Planner authoring, active revision selection, Task DAG execution, approval/activation, and successor/replanning runtime are not. Admission makes a proposal durable only; it does not approve, activate, select, schedule, or dispatch it, and current provider availability is not an admission condition. PlanRevision binds no exact DataProfile. Executable `SCIENTIFIC`, `GRAPH`, and `SYNTHESIS` workflows, specialist DataProfile-selection plumbing, ScientificInvestigationRun, Hypothesis Analyst feasibility, Hypothesis admission, InvestigationPlan/Protocol, EvidenceRequest, canonical ExecutionRun/AnalysisFrame use, canonical Evidence admission, protected evaluation, typed outcomes, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
...
`SYNTHESIS`. The PlanRevision V1 domain contract, exact SQLite persistence, and
application-owned proposal admission are **Implemented**. Planner authoring,
approval, activation, active selection, lifecycle progression, and runtime use
remain **Deferred**.
