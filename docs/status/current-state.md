...
implementation?** It describes source and tests inspected on 2026-08-11 at
fetched `origin/main` `d50fbd63d9bef9e46b5f6e546cd2b33ea21879e2`. A
...
| Planner behavior, native conversation history, and `PlannerWorkOutcome` consumption | **Implemented** | The bounded library graph performs typed request understanding, successor-state Objective and Assumption handling, Task creation, typed capability selection, dispatcher invocation, Task-identity-checked outcome consumption, lifecycle completion/failure, Evidence-only follow-up answering, and controlled human-facing responses. One complete model-backed Planner run is retained as a `ConversationTurn` of native PydanticAI `ModelMessage` values; append-only `ConversationHistory` is supplied to later request-understanding model calls. Conversation remains non-authoritative and is not added to the Evidence-only answer input. Planner still does not admit Evidence; the separate M3-A application service can project a real admitted Evidence reference for later runtime composition. The current Task is bounded M1-A state, not a canonical four-kind Task. |
...
| Multi-provider model construction | **Partially implemented** | Current source selects OpenAI, Google/Gemini, or Anthropic from required provider/name/key configuration resolved workspace-first and then from environment. No live provider call is verified by this documentation milestone. Focused tests retain older configuration/factory assumptions and currently fail in four places; this is named baseline debt, not proof of a supported multi-provider end-to-end runtime. |
...
| Canonical planning and scientific runtime | **Deferred** | Canonical Task kinds, PlanRevision and Task DAG approval/activation, ScientificInvestigationRun, Hypothesis Analyst feasibility, Hypothesis admission, InvestigationPlan/Protocol, EvidenceRequest, ExecutionRun, AnalysisFrame, canonical Evidence admission, protected evaluation, typed outcomes, governance, Discovery admission, and validity propagation are required by MVP-v2 but are not supported workflows. |
...
Result: **4 passed, 4 failed**. Native conversation-history tests pass. The
four failures are pre-existing post-merge test drift: one Planner model test
constructs `ModelConfig` without the now-required provider, two bootstrap
configuration tests omit the provider, and one LLM factory test references the
removed `OpenAICompatibleAgentFactory` name. This documentation-only milestone
does not change production code or tests to mask that baseline debt.
