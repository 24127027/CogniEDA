...
| Runtime Session and conversation continuity | **Implemented** | At the in-process `Application` boundary, one immutable successor `Session` owns the active `SessionFrame` and an ordered append-only `ConversationHistory` as separate components. Each submitted Human message and returned Planner response forms one normalized turn; provider messages, tool protocols, retries, and intermediate model calls are excluded. The next turn receives the successor frame and a non-authoritative conversation projection for request understanding. |
...
| Runtime entry boundary | **Partially implemented** | An editable uv tool installation exposes `cognieda [PATH]`; `python -m cognieda` delegates to the same package entrypoint. Help parsing is bootstrap-free. Runtime bootstrap composes the registry, Data Explorer provider, dispatcher, M1-B Planner, model factory, workspace-local agent tooling, and one retained in-process Session. The development REPL retains successor `SessionFrame` and normalized conversation state across turns, but it does not supply composed authoritative dataset execution context or restart-safe recovery, so it is not complete M5-A or a supported product CLI. |
