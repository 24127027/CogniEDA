...
| Objective and Assumption | **Implemented** | `Objective` and planning-only `Assumption` each have stable UUID identity and non-empty text. Bounded Planner returns explicit results for standalone Objective or Assumption changes. A DATA request may instead place a reused or newly proposed Objective inside the same transient PlanDraft as its Task proposal; neither becomes authoritative before exact Human approval. |
...
| PlanDraft approval and authoritative plan commit | **Implemented** | Planner proposes one immutable transient non-FCO `PlanDraft` containing the exact Objective proposal, TaskDraft values, coordination, dependencies, and a structural `sha256:` fingerprint. Application retains at most the current in-process draft and accepts explicit `/approve <fingerprint>` or `/reject <fingerprint>`. Rejection and non-exact approval create no authoritative plan state. Exact approval invokes one application-owned SQLite transaction that resolves or creates the Objective, constructs canonical Tasks, revalidates the PlanRevision against those Tasks, persists the immutable revision, and creates the sole explicit active-revision selection. Injected failure rolls back Objective, Tasks, revision, and selection together. Draft persistence, restart-safe approval, approval modes, and replanning replacement are **Deferred**. |
...
| Planner behavior, native conversation history, and `PlannerWorkOutcome` consumption | **Implemented** | The bounded graph receives read-only `PlanningContext`, performs typed request understanding, returns standalone Objective or Assumption results, or constructs one transient Objective-plus-DATA PlanDraft without dispatch. Application handles exact approval, authoritative commit, and active-plan execution, then returns the normalized role-native DATA result to Planner for a Human-facing response. Planner graph state and output contain no SessionFrame, Planner is not an executor, and response synthesis is not Task work. Native model-message history remains append-only and separate from research authority. Planner does not admit computed DATA output as Evidence or create executable `SCIENTIFIC` or `GRAPH` work. |
...
| Approved DATA runtime composition | **Implemented** | A verified in-process Application test spans Human request, Planner Objective resolution and one-DATA-Task PlanDraft, exact approval, atomic Objective/Task/PlanRevision activation, active-plan eligibility, authoritative DataProfile and dataset-binding resolution, dispatcher and registered Data Explorer execution against a real CSV, completed Task persistence, and Planner response. Before approval and after rejection the authoritative plan tables remain empty. The computed result is returned directly to Planner and Evidence count remains zero. The runtime also verifies deterministic `T1 -> T2` dependency progression, order/priority subordinate to prerequisites, no completed-Task rerun, and typed blocking when capability is unavailable. |
...
PlanDraft(plan_draft_id, objective, task_drafts, dependencies, fingerprint)
...
exact transient PlanDraft construction without pre-approval mutation or
dispatch, all three data capabilities, rejection and non-exact approval,
...
execution from approved PlanDraft through active PlanRevision, Data Explorer,
