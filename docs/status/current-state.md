...
| Bounded materialized SessionFrame | **Implemented** | The current M1-A value retains one optional materialized Objective, ordered read-only materialized Assumption, Task, and Evidence collections, and one optional materialized DataProfile. Controlled seams return validated replacements. It rejects duplicate IDs, orphan Evidence, Evidence for any Task not exactly `COMPLETED`, Evidence without a DataProfile, and Evidence for a different profile. An adjacent immutable `SessionFrameMembership` value contract carries typed UUID membership references and active Objective/DataProfile selectors, and the materialized frame projects deterministically into it without fabricating Hypothesis or Discovery membership. The active frame remains materialized and has no `session_frame_id`, Objective-bound session identity, successor lineage, purpose/mode binding, persistence/runtime cutover, or runtime reload authority. |
