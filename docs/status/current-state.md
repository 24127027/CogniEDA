...
| PlanRevision V1 domain contract and candidate validation | **Implemented** | Immutable non-FCO `PlanRevision`, `PlanTaskBinding`, and `PlanDependency` values provide one binding per member Task, binding-owned non-null execution capability/non-negative tie-permitting order rank/LOW-NORMAL-HIGH priority, explicit Objective-scoped acyclic dependencies, deterministic canonical ordering, and a `sha256:` fingerprint bound to contract version and plan content. The side-effect-free application validator resolves authoritative Objective and Task records, rejects duplicates, missing or cross-Objective Tasks, invalid kind/capability combinations, non-member or cyclic edges, non-canonical representation, and fingerprint mismatch, then returns the exact canonical candidate without persistence. Execution-owned `Capability` is the only routing requirement; provider availability is not a validation condition. PlanRevision and its bindings contain no role or provider identity, DataProfile identity, stopping/replan policy, approval, or activation state. |
...
| Canonical planning and scientific runtime | **Deferred** | The PlanRevision V1 domain contract, pure candidate validation, and append-only SQLite repository foundation exist, but Planner authoring, human approval, exact post-approval revalidation and persistence, active revision selection, activation, Task DAG execution, and successor/replanning runtime do not. Validation performs no authoritative persistence side effect, and no unapproved PlanRevision candidate is persisted through an application path. PlanRevision binds no exact DataProfile. Executable `SCIENTIFIC` and `GRAPH` workflows, specialist DataProfile-selection plumbing, ScientificInvestigationRun, Hypothesis Analyst feasibility, Hypothesis admission, InvestigationPlan/Protocol, EvidenceRequest, canonical ExecutionRun/AnalysisFrame use, canonical Evidence admission, protected evaluation, typed outcomes, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
...
approval, exact post-approval validation and persistence, activation, active
