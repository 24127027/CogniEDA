...
| Objective and Assumption | **Implemented** | Immutable `Objective` and planning-only `Assumption` each have stable UUID identity and non-empty text. Planner may return a transient candidate Plan containing the current or a newly proposed Objective, but it cannot create an Assumption. Human approval of the exact Plan may atomically admit its new Objective; every Plan Assumption must already exist with exact identity and content and is never auto-created by Plan approval. |
...
| Bounded persistence | **Verified on SQLite** | Concrete models, sessions, and repositories round-trip minimum research state, immutable Plan state, and objective-scoped active Plan selection on fresh SQLite. Plan retains normalized `plans`, `plan_assumptions`, `plan_tasks`, and atomic-edge `plan_dependencies` rows; repository writes flatten grouped dependencies and reads regroup them by prerequisite. One admission transaction validates and stages an approved new Objective, new Tasks, exact Plan, and `objective_id -> plan_id` active pointer, then commits all or rolls all back. The append-only Plan repository exposes no update or delete surface, and replacing the active pointer leaves old Plans immutable. Durable upgrade of legacy rows and restart/recovery are not claimed. |
| Human Plan review and admission | **Verified on SQLite** | Frozen `PlanReviewDecision` binds `APPROVE`, `REJECT`, or `REVISE` to one exact candidate Plan ID; reject and revise require and preserve exact Human feedback. Application retains one exact transient candidate bundle. Reject/revise create no authoritative writes. Approve requires every Assumption to be already admitted with unchanged content, reuses exact existing Objective/Task identity, rejects semantic collisions, admits new Objective and Tasks when needed, persists the Plan once, and activates it atomically. Conversation text alone is not admission authority. Pending review and feedback are not durable. |
...
| Canonical planning and scientific runtime | **Deferred** | Plan domain, pure validation, transient candidate authoring, typed Human review, atomic approved-bundle admission, and objective-scoped active selection exist. No active Plan executes: Task DAG selection/execution, successor/replanning orchestration, and final LangGraph composition remain absent, and Plan binds no exact DataProfile. The scientific runtime, canonical Evidence admission, protected evaluation, governance, Discovery admission, and validity propagation remain unsupported workflows required by MVP-v2. |
| Plan lifecycle and replanning | **Partially implemented** | Exact Human approval and objective-scoped active selection are implemented, and successor activation does not mutate the old Plan. Plan completion, interruption, successor-authoring policy, pending-review recovery, and actual-cause taxonomy remain **Deferred**. Scientific stopping remains InvestigationProtocol-owned, and bounded execution stopping remains work-order-owned. |
...
| Runtime entry boundary | **Partially implemented** | Runtime bootstrap composes fresh-SQLite metadata, the registry, Data Explorer provider, dispatcher, Phase 2 Planner, model factory, and workspace-local agent tooling. One in-process `Application` owns the current SessionFrame, exact transient candidate, typed review method, and native conversation history. It resolves the active Plan for the frame's exact current Objective and materializes it into `PlannerContext`; `continue_execution` remains validation-only and invokes no executor. The REPL does not expose typed Plan review, persisted state is not recovered, and no Plan execution loop exists, so this is not a supported product CLI. |
...
candidate authoring, typed Human review, commit-boundary validation and atomic
persistence, and objective-scoped activation are **Implemented** and **Verified
on SQLite** at their bounded surfaces. Lifecycle progression, durable
pending-review recovery, and runtime execution remain **Deferred**.
...
Runtime tests prove that candidate state is not admitted before exact typed
approval, reject/revise write nothing, approved state commits atomically, and
the exact objective-scoped active Plan reaches `PlannerContext` alongside all
six materialized SessionFrame member categories. Layer tests
