...
| Bounded materialized SessionFrame | **Verified on SQLite** | The current M1-A value retains one optional materialized Objective, ordered read-only materialized Assumption, Hypothesis, Evidence, and Discovery collections, and one optional materialized DataProfile. `SessionFrameRepository` appends scoped snapshots and defines current as the latest committed snapshot in an exact workspace scope; absence yields deterministic empty state. `Application` writes and reads through that repository, and `PlannerContextProvider` freshly reads the same persisted current frame on every invocation before exact-copying all six member categories into immutable `PlannerContext` and adding the objective-scoped active Plan separately. Restart reconstruction from the same database and scope is verified. The frame rejects duplicate IDs, Evidence without a DataProfile, and Evidence for a different profile. It does not retain Tasks or own Task lifecycle. The materialized value still has no canonical `session_frame_id`, typed reference manifest, active selectors, successor lineage, or purpose/mode binding. |
...
| Conversational Plan authorization and candidate lifecycle | **Implemented** | Planner-owned in-process `PlannerState` retains one self-contained exact candidate Plan and native model history outside `PlannerContext`; no parallel candidate or proposed Task tuple exists. Natural-language Human turns resume through the Planner-owned LangGraph interrupt state; typed Planner results retain, replace, explicitly discard, or authorize the exact candidate without keyword or regex parsing. The deterministic admission node calls `PlanAdmissionService.admit(plan)`; success clears the candidate, while failure retains the exact Plan and its embedded Tasks with a controlled error. Exact admission and objective-scoped active Plan selection are **Verified on SQLite**. Checkpoints are process-local and durable restart recovery is **Deferred**. |
...
| Planner cognitive core and native conversation history | **Implemented** | Planner directly owns one typed PydanticAI Agent and invokes `plan_or_answer` exactly once per graph turn with exact typed `PlannerDeps`. The latest Human text is the current user prompt. Graph-owned prior messages are supplied separately as native `message_history`, preserving chronology without making it authoritative; only current-run `new_messages()` are appended. `PlannerContextProvider` freshly reads scoped persisted SessionFrame authority and the active Plan for each invocation, then supplies deterministic context through the current-run instruction channel. Plan validates its own embedded Tasks; model/context validation fails closed on unknown or changed Assumptions. `PlannerContext` contains active Plan coordination plus readable research state, including Hypotheses and excluding generic Tasks. The model-visible contracts contain no Capability or execution routing. Planner itself performs no persistence, dispatch, Hypothesis authoring, or Evidence admission. |
...
| Retained runtime composition | **Partially implemented** | Bootstrap composes one process-local Planner with a workspace-scoped persisted SessionFrame repository, fresh-context provider, and deterministic admission service. Planner owns native history, candidate review, interrupt/resume, and exact admission routing; Application invokes Planner and maps its outcome to presentation-only events. Materialized SessionFrame current state is restart-readable from the same database and scope; LangGraph history/candidate state is not. No supported Planner-to-real-Data-Explorer-to-Evidence-to-SessionFrame workflow is composed. |
...
| Runtime entry boundary | **Partially implemented** | Runtime bootstrap composes SQLite metadata and session access, a workspace-scoped authoritative `SessionFrameRepository`, delegation infrastructure, a fully wired Planner, model factory, EventBus, and workspace-local agent tooling. Application and the fresh-context provider share the repository; Planner owns only its LangGraph, checkpointer/thread UUID, native history, and candidate lifecycle. `Application.submit_message()` delegates normal text to `Planner.handle_message()` and maps typed turn outcomes to presentation events. `PlanProposed` carries only the self-contained Plan; EventBus stores no research state or candidate authority. Typed authorization invokes application-owned admission through the deterministic graph node; `continue_execution` for an already-active Plan returns a visible no-execution result and never dispatches. Persisted graph state is not recovered and no Plan execution loop exists, so this is not a supported product CLI. |
...
```text
uv run pytest -q \
  tests/schemas tests/agents/planner \
...
Result: **202 passed**. Changed-boundary Ruff, `compileall`, documentation links,
and obsolete-contract searches also pass. This is bounded evidence, not a claim
that deferred Plan execution, durable Planner lifecycle recovery, or scientific
workflows exist.
