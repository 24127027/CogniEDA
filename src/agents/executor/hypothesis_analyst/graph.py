...
def build_graph(
    *,
    ...
    builder.add_conditional_edges(
        "plan_de_requests",
        lambda state: "dispatch_to_de"
        if state["de_capability_requests"]
        else "handle_failure_and_logs",
        {
    ...
    builder.add_conditional_edges(
        "dispatch_to_de",
        lambda state: "evaluate_evidence"
        if state["collected_evidence"]
        else "plan_de_requests",
        {
    ...
    builder.add_conditional_edges(
        "assess_scientific_value",
        lambda state: "request_admission"
    ...
    builder.add_conditional_edges(
        ...
        if state["discovery_draft"] is not None
        else "handle_failure_and_logs",
        {
            "compile_result": "compile_result",
            "handle_failure_and_logs": "handle_failure_and_logs",
