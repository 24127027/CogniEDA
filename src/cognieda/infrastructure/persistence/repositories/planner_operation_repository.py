...
from cognieda.infrastructure.persistence.models import PlannerOperationRecord
...
from cognieda.schemas.planner_operations import PlannerOperation
    ...
    def create_batch(self, operations: list[PlannerOperation]) -> list[PlannerOperation]:
        ...
        try:
            for operation in operations:
                if self._session.get(PlannerOperationRecord, operation.operation_id) is not None:
                    raise ValueError(f"PlannerOperation already exists: {operation.operation_id}")
                record = PlannerOperationRecord(
                    **schema_to_record_payload(
                        operation,
                        json_fields=PLANNER_OPERATION_JSON_FIELDS,
    ...
    def set_approval_state_batch(
        ...
        *,
        ...
        if any(record is None for record in records) or any(
            record.approval_state != expected_state for record in records if record is not None
        ):
        ...
        for record in records:
            ...
            self._session.refresh(record)
        return [
            record_to_schema(PlannerOperation, record) for record in records if record is not None
        ]
