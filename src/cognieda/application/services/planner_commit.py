    ...
    *,
    ...
    try:
        ...
        return PlannerCommitResult(
            committed_operation_ids=[record.operation_id for record in committable_records],
...
def _apply_execution_admission_bundle(
    ...
    if any(
        record.approval_state != PlannerOperationApprovalState.NOT_REQUIRED for record in records
    ):
...
def _apply_update_assumption_state(
    ...
    update = AssumptionUpdate(**payload.model_dump(exclude={"assumption_id"}, exclude_none=True))
    _require_update_payload(update.model_fields_set, operation.operation_type.value)
...
def _apply_update_objective(session: Session, operation: PlannerOperationRecord) -> None:
    ...
    update = ObjectiveUpdate(**payload.model_dump(exclude={"objective_id"}, exclude_none=True))
    _require_update_payload(update.model_fields_set, operation.operation_type.value)
...
def _require_payload_target_matches(
    ...
    if operation.target_object_id is not None and operation.target_object_id != payload_target_id:
        raise ValueError(f"{operation_name} payload target does not match target_object_id.")
