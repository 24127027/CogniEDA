...
def test_concurrent_evidence_admission_claim_has_real_overlap(db_session):
    ...
    def claim_worker(worker_id):
        ...
        try:
            ...
            res = svc.claim_evidence_admission(
                run_id,
                worker_id,
                run.attempt_version,
                datetime.now(UTC) + timedelta(minutes=5),
    ...
    assert len(successes) == 1, "Exactly one thread should claim Evidence admission."


def test_cancellation_beats_evidence_admission(db_session):
    database_url = str(db_session.get_bind().url)
    run_id_str = _create_admitted_run(db_session)
    from uuid import UUID
    ...
    claim_res = finalize_svc.claim_evidence_admission(
        run_id,
        "finalizer-A",
        run.attempt_version,
        datetime.now(UTC) + timedelta(minutes=5),
    ...
    assert claim_res is False, "Evidence admission must fail after cancellation."


def test_evidence_admission_beats_cancellation(db_session):
    database_url = str(db_session.get_bind().url)
    run_id_str = _create_admitted_run(db_session)
    from uuid import UUID
    ...
    run = finalize_session.get(ExecutionRunRecord, run_id)
    # Evidence admission records success using the fenced stage transition.
    finalize_svc.stage_admit_evidence(
        execution_run_id=run_id,
...
def test_stale_evidence_admission_owner_is_fenced_after_reclaim(db_session):
    database_url = str(db_session.get_bind().url)
    run_id_str = _create_admitted_run(db_session)
    from uuid import UUID
    ...
    claim_a = svca.claim_evidence_admission(
        run_id,
        "finalizer-A",
        run.attempt_version,
    ...
    run_a.finalization_expires_at = datetime.now(UTC) - timedelta(minutes=1)
    session_a.add(run_a)
    session_a.commit()
    ...
    claim_b = svcb.claim_evidence_admission(
        run_id,
        "finalizer-B",
        run.attempt_version,
        datetime.now(UTC) + timedelta(minutes=5),
    ...
    res_a = svca_wake.stage_admit_evidence(
        execution_run_id=run_id,
        finalizer_owner_id="finalizer-A",
    ...
    res_b = svcb.stage_admit_evidence(
        execution_run_id=run_id,
